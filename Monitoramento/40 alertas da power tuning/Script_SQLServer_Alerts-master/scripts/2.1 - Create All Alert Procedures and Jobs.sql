...
SET NOCOUNT ON
	...
	SELECT '<br />
			<br />'			
			+ 
				'<a href="https://www.hostweb.com.br/" target=”_blank”> 
					<img	src="https://www.hostweb.com.br/wp-content/uploads/2023/07/Logo-wordpress.png"
...
BEGIN
	...
	select A.name [Nm_Database],B.Data_Backup Data_Backup
	into #Alert_Log_Backup_Database
	from sys.databases A
		left Join #Ultimo_Backup_Log_Database B on A.name = B.[Nm_Database]
	where [recovery_model_desc] = 'FULL'
	--and A.name not in (	select database_Name from sys.availability_databases_cluster) -- backups realizados na réplica do alwaysON
	

	--	Do we have backup problems?	
	IF EXISTS (SELECT * FROM #Alert_Log_Backup_Database
				Where  DATEDIFF(HH,isnull(Data_Backup,getdate()-2),GETDATE()) >= @Vl_Parameter )
	...
	BEGIN
		...
		BEGIN
			...
			SELECT [Nm_Database] [Database],isnull(CONVERT(VARCHAR(50), Data_Backup, 120), 'Não temos registros de backup nas últimas 48 horas')[Ultimo Backup Executado]
			INTO ##Email_HTML	
