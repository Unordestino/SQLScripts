/*******************************************
 ...
 Hands On: Copiar Login com senha
********************************************/
use master
go

/**********************************************
 Gera comandos com membros dos Server Roles
***********************************************/
SELECT sp1.[name] AS 'login', sp2.[name] AS 'role',
'EXEC sp_addsrvrolemember ''' + sp1.[name] + ''', ''' + sp2.[name] + '''' as Ate_SQL2008,
'ALTER SERVER ROLE ' + sp2.[name] + ' ADD MEMBER [' + sp1.[name] + ']' as SQL2012_EmDiante
...
WHERE sp1.[name] not like 'NT SERVICE%' and sp1.[name] <> 'sa'
ORDER BY LOGIN


/**********************************************************************
 Criar Stored Procedures
 - Para gerar os logins executar após crias as Stored Procedures:
...
IF (@login_name IS NULL)
  DECLARE login_curs CURSOR FOR
	  ...
	  and p.name not like 'NT SERVICE%' and p.name not like 'NT AUTHORITY%' and p.name not like '##%##'
ELSE
  DECLARE login_curs CURSOR FOR
